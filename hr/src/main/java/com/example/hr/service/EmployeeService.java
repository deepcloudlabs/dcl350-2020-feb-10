...
public class EmployeeService {
	...
	public Employee findEmployeeByIdentity(String identity) {
		return empRepo.findOneByIdentityNo(identity)
				.orElseThrow(() -> 
				new BaseException(
						"Cannot find the employee",
						ErrorCode.EMPLOYEE_NOT_FOUND,
						"b8c54f20-fac3-423c-b611-a95227ec0249"));
	}
	...
	public Employee addEmployee(Employee emp) {
		...
		if (employee.isPresent())
			throw new BaseException(
					"Employee already exists with the same identity no",
					ErrorCode.ALREADY_EXISTING_EMPLOYEE,
					"1cc40c5d-bb30-4e71-9ed3-1e5acab92151") ;
	...
	public Employee updateEmployee(Employee emp) {
		...
		if (!employee.isPresent())
			throw new BaseException(
					"Cannot find the employee to update",
					ErrorCode.EMPLOYEE_NOT_FOUND,
	...
	public Employee removeEmployeeByIdentity(String identity) {
		Optional<Employee> employee = 
				empRepo.findOneByIdentityNo(identity);
		if(!employee.isPresent())
			throw new BaseException(
					"Cannot find the employee to remove",
					ErrorCode.EMPLOYEE_NOT_FOUND,
